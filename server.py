...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def log_message(self, fmt, *args):
        ...
        print(f'  [{args[1]}] {self.command} {args[0]}')
    ...
    def do_GET(self):
        try:
            ...
            # Last-resort catch — nothing should ever bubble up to the server
            traceback.print_exc()
            try:
    ...
    def send_head(self):
        ...
        if clean_path.endswith(('.js', '.css', '.png', '.ico')):
            import os, mimetypes
...
with ReuseServer(('', PORT), Handler) as httpd:
    print(f'PostPrint server → http://localhost:{PORT}')
    try:
        httpd.serve_forever()
